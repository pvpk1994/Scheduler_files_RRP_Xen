...
if [ $(whoami) != "root" ]; then
	...
	#cat list_info.txt | awk '{ for(counter = 0; counter < 3; counter++) {if ($10=="arinc_simple") {print $7}}}' >$UUID_INFO
	cat $(pwd)/pool_uuid/list_info.txt | awk '{if($10=="arinc_simple") {print $11}}' >${POOLID_INFO}
        for i in $(seq 1 $NUM_RECORDS); do
		 cat $(pwd)/pool_uuid/list_info.txt | awk '{if($10=="arinc_simple") {print $7}}' >$UUID_INFO
	done
	if [ $? -eq 0 ]; then
