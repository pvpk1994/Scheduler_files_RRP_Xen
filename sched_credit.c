...
/* Print time */
#define printtime() (printk("Time in Credit Scheduler running on CPU %d : %3ld.%3ld : %-19s \n",smp_processor_id(),  \
...
csched_init_pdata(const struct scheduler *ops, void *pdata, int cpu)
{
    ...
    printtime();
    printk(" csched_init_pdata function invoked on CPU: %d\n",cpu);
    /*
    ...
    spin_unlock_irqrestore(&prv->lock, flags);
    printk("Leaving csched_init_pdata from cpu: %d\n",cpu);
}
                    ...
                    void *pdata, void *vdata)
{
   ...
   printtime();
    printk("Switching schedule using csched_switch_sched on cpu:%d\n",cpu);
...
csched_cpu_pick(const struct scheduler *ops, struct vcpu *vc)
{
    ...
    printtime();
    printk("cshed_cpu_pick invoked for vcpu#:%d\n",vc->vcpu_id);
    /*
...
csched_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    ...
    if ( svc == NULL )
        return NULL;
      printk("csched_alloc_vdata invoked with a vcpu#: %d\n",vc->vcpu_id);
	...
	printk("Leaving csched_alloc_vdata with vcpu from domain %d\n", dd);
    return svc;
}
...
csched_vcpu_insert(const struct scheduler *ops, struct vcpu *vc)
{
    ...
    BUG_ON( is_idle_vcpu(vc) );
   printtime();
   printk("csched_vcpu_insert invoked on VPCU# %d\n",vc->vcpu_id);
	...
	printk("Leaving csched_vcpu_insert\n");
}
...
csched_alloc_domdata(const struct scheduler *ops, struct domain *dom)
{
    ...
    if ( sdom == NULL )
        return ERR_PTR(-ENOMEM);
    // printtime();
     printk("csched_alloc_domdata invoked on domain# : %d\n",dom->domain_id);
    /* Initialize credit and weight */
    INIT_LIST_HEAD(&sdom->active_vcpu);
    INIT_LIST_HEAD(&sdom->active_sdom_elem);
    sdom->dom = dom;
    sdom->weight = CSCHED_DEFAULT_WEIGHT;
   printk("Leaving csched_alloc_domdata\n");
    return sdom;
}
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    //printtime();
    //printk("csched_schedule invoked on CPU# %d and on VCPU# %d\n",cpu,scurr->vcpu->vcpu_id);
    /*
...
out:
    ...
    ret.time = (is_idle_vcpu(snext->vcpu) ?
                -1 : tslice);
    ret.task = snext->vcpu;
    printk("Time elapsed in credit scheduler: %d\n",ret.time);
    CSCHED_VCPU_CHECK(ret.task);
//	printk("Leaving csched_schedule\n");
    return ret;
}
...
csched_global_init(void)
{
	...
	// printk("Running Credit Scheduler on CPU: %d\n",smp_processor_id());
    return 0;
}
...
csched_init(struct scheduler *ops)
{
   ...
   printk("Running Credit Scheduler in csched_init on CPU: %d\n",smp_processor_id());
