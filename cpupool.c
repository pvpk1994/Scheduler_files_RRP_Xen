...
{
    printk("The scheduler id in cpupool_create %d\n", sched_id);
    struct cpupool *c;
    struct cpupool **q;
    int last = 0;
    *perr = -ENOMEM;
    //if(sched_id==3)
    //	BUG();
        //thi bug is touched when sched=\"aaf\"
    ...
    //cpupool_d
    printk("cpupool_create(pool=%d,sched=%u)\n", poolid, sched_id);
...
int cpupool_do_sysctl(struct xen_sysctl_cpupool_op *op)
{
    ...
    {
	printk("Pool Creat in the xen kernel is invoked.\n");
        int poolid;
